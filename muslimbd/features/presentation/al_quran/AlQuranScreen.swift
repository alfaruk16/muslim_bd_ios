...
struct AlQuranScreen: View {
    let category: CategoryDtoItem
    
    @Environment(\.dismiss) var dismiss
    
    @StateObject var viewmodel = AlQuranViewModel()
                ...
                ScrollView{
                    LazyVStack{
                        ForEach(state.alQuran.data ?? [], id: \.id){ item in
                            NavigationLink(destination: SurahScreen(category: category, surahItem: item, audioPlayer: audioPlayer), label: {
                                SurahName(surah: item, audioPlayer: audioPlayer)
                            })
