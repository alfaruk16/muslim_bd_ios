//
//  CategoryScreen.swift
//  muslimbd
//
//  Created by Al Faruk on 12/1/25.
//

import SwiftUI

struct SubCategoryScreen: View {
    let category: CategoryDtoItem
    
    @Environment(\.dismiss) var dismiss
    
    @StateObject var viewmodel = SubCategoryViewModel()
    
    let width = UIScreen.main.bounds.width / 1.75
    
    
    var body: some View {
        let state = viewmodel.state
        
        NavigationStack {
            
            ZStack (alignment: .top){
                BismillahView(width: width, icon: .bismillah)
                
                ScrollView{
                    LazyVStack{
                        
                        if(state.subCategories.data != nil){
                            ForEach(state.subCategories.data ?? [], id: \.id){ sub in
                                let subCategory = CategoryDtoItem(name: sub.name, imageURL: sub.imageURL, contentBaseURL: sub.contentBaseURL, id: sub.id, icon: sub.icon, hasSubCategory: true, category: category.id)
                                
                                NavigationLink(destination: CategoryScreen(category: subCategory), label: {
                                    SubCategoryItem(subCategory: sub)
                                })
                            }
                        }
                        
                        Spacer().frame(height: 20)
                        
                    }.padding(.top, width - 30).padding(.horizontal, 10)
                    
                }
                
            }.frame(maxHeight: .infinity, alignment: .top)
                .navigationBarTitleDisplayMode(.inline)
                .toolbar{
                    ToolbarItem(placement: .principal) {
                        AppBar(title: category.name ?? "", icon: category.icon, image: (category.contentBaseURL ?? "") + (category.imageURL ?? ""), dismiss: { dismiss() })
                    }
                }
        }.navigationBarBackButtonHidden(true)
            .onAppear(perform: {
                viewmodel.initiate(category: category)
            })
        
    }
}


#Preview {
    SubCategoryScreen(category: CategoryDtoItem(name: "Category", icon: .home))
}
